...
\begin{equation}
f(N_{i}, \mathbf{c}_{i}) = ymax(\mathbf{c}_{i})(1-exp(\alpha(\mathbf{c}_{i}) + \beta(\mathbf{c}_{i}) N_{i})) + \varepsilon_{i}. 
\end{equation}
...
field_alpha + field_beta + field_ymax + field_m_error + vis_optN_cell + plot_layout(ncol = 2)
```
...
$\alpha_{1}_{i,j} = r^{\alpha}_{i,j} \alpha_{i,j}$, $\alpha_{2}_{i,j} = (1- r^{\alpha}_{i,j}) \alpha_{i,j}$,
$\beta_{1}_{i,j} = r^{\beta}_{i,j} \beta_{i,j}$, $\beta_{2}_{i,j} = (1- r^{\beta}_{i,j}) \beta_{i,j}$,
$ymax_{1}_{i,j} = r^{ymax}_{i,j} ymax_{i,j}$, and $ymax_{2}_{i,j} = (1- r^{ymax}_{i,j}) ymax_{i,j}$. The codes used to implement the abovementioned parameter splitting is available at https://github.com/tmieno2/ML_VRA.git.

<span style = "color: blue;"> Shun, provide the link to Mandeley data and cite it </span>
