...
$\alpha^{1}_{i,j} = r^{\alpha}_{i,j} \alpha_{i,j}$, $\alpha^{2}_{i,j} = (1- r^{\alpha}_{i,j}) \alpha_{i,j}$,
$\beta^{1}_{i,j} = r^{\beta}_{i,j} \beta_{i,j}$, $\beta^{2}_{i,j} = (1- r^{\beta}_{i,j}) \beta_{i,j}$,
$ymax^{1}_{i,j} = r^{ymax}_{i,j} ymax_{i,j}$, and $ymax^{2}_{i,j} = (1- r^{ymax}_{i,j}) ymax_{i,j}$. The codes used to implement the abovementioned parameter splitting is available at https://github.com/tmieno2/ML_VRA.git.
