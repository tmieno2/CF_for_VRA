...
The size of random yield error $\varepsilon$ should be coordinated with the potential yield level for a site $(i,j)$.  To do so, we first created spatially autocorrelated weights $r^{\varepsilon}_{i,j} \in (-1,1)$. After $\alpha_{i,j}$, $\beta_{i,j}$, $ymax_{i,j}$, and $r^{\varepsilon}_{i,j}$ were generated, then $\varepsilon_{i,j}$ was determined by interacting $r^{\varepsilon}_{i,j}$ with the deterministic part of yield (i.e., $ymax_{i,j}(1-exp(\alpha_{i,j} + \beta_{i,j} \cdot N_{i}))$). This process can be expressed by rewriting Eq. \@ref(eq:eqn3) as follows. The observed yield at site $(i, j)$ (i.e., $y_{i,j}$) is:

\begin{align}
y_{i,j} 
&=  ymax_{i,j}(1-exp(\alpha_{i,j} + \beta_{i,j} \cdot N_{i}))(1+r^{\varepsilon}_{i,j}) \\
&=  ymax_{i,j}(1-exp(\alpha_{i,j} + \beta_{i,j} \cdot N_{i})) +  ymax_{i,j}(1-exp(\alpha_{i,j} + \beta_{i,j} \cdot N_{i})) \cdot r^{\varepsilon}_{i,j} \\
...
The figure XX plots the predicted yields against the true yield without error in one of the one thousand simulation rounds by ML method and modeling scenario. The predicted points clustered around the red 1-to-1 line, meaning that the prediction-oriented ML methods predicted yields well.

However, as mentioned in section 4.2, please note that even though the yield prediction accuracy evaluated at the actually applied N rate is , it does not necessary mean good performance in predicting yield “response” function which requires counterfactual prediction.  


```{r vis-y-pred, fig.width=7, fig.height=7.5, fig.cap = "Figure E.1: True yields without error and predicted yields"}
vis_y_pred
```
...
In this section, we present the results of EONR estimation and yield prediction with different size of random yield error ($\varepsilon$). We created two additional yield datasets	that are only different in the size of $\varepsilon$ from the yield datasets we used in the study by changing the variogram parameter of $psill$ for $r^{\varepsilon}_{i,j}$. Half size of $psill$ value ($0.0075$) was used to create yield datasets with low yield error, and one and a half size of $psill$ value ($0.0225$) was used to create yield datasets with high yield error. Figure G.1 illustrates examples of the generated low and high $\varepsilon$. 

```{r fig-nExp-yield, fig.width=7, fig.height=6, fig.cap = "Figure G.1: An example of experimental design of N and generated yield at the subplot resolution", cache = TRUE}
field_m_error_low | field_m_error_high
```

\newpage

Using those new yield datasets, we conducted one hundred rounds of simulations respectively. The results of EONR estimation and yield prediction are shown in Table G.1 and G.2. Table G.3 summarizes the relationship between yield and EONR prediction RMSE.  

As the size of yield error increases, EONR estimation and yield prediction accuracy becomes improved. However, qualitative results about the comparative advantage of CF over other models remain the same, and Table G.3 consistently illustrates that good performance in yield prediction does not necessary indicate good performance in EONR estimation. 
