...
```{r ct-ex, fig.width=5, fig.height=5, fig.align="center", fig.cap = "An illustrative example of a causal tree"}
...
```{r fig-parameters, fig.width=7, fig.height=9, fig.cap = "An Illustrative wxample of spatial distributions of field characteristics", cache = TRUE}
...
# Appendix C: Training Accuracy of the ML methods
...
Table \@ref(tab:table-optN-train) shows the mean RMSE of the EONR estimation and $\hat{\pi}_{def}$ over the one thousand simulations using the training datasets as testing datasets. Overall, the performances of each ML method did not change between training and testing datasets. Regarding CNN, the identified site-specific yield response functions were perfectly linear as we confirmed in the testing results. 

```{r table-optN-train, tab.cap = "Mean RMSE of EONR (kg/ha) estimation and profit-deficit ($/ha) by ML methods and modeling scenarios on training datasets"}
report_table_optN_train
```

Table \@ref(tab:table-y-train) shows the mean RMSE of yield prediction over the one thousand simulations using the training datasets as testing datasets. Interestingly, the performances of the prediction-oriented ML methods in predicting yields were relatively improved compared with the test results. Despite the improvement of prediction-oriented ML methods in yield prediction accuracy, it did not result in the more accurate EONR estimates. This supports our argument that the model’s yield prediction accuracy has little to do with its EONR prediction accuracy.


```{r table-y-train, tab.cap = "Mean RMSE of yield prediction (kg/ha) by ML methods and modeling scenarios on training datasets"}
