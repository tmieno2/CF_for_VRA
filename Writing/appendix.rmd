...
## Appendix I: Simulations with Different Size of Yield Disturbance Term.
In this section, we present the results of EONR estimation and yield prediction with different size of yield disturbance term ($\varepsilon$). We created two additional yield datasets	that are only different in the size of $\varepsilon$ from the yield datasets we used in the study. This can be done by just changing the variogram parameter of $psill$ for $\varepsilon$. $0.002$ of $psill$ value was used to create low $\varepsilon$, and $0.028$ of $psill$ value was used to create high $\varepsilon$. Figure I.1 illustrates examples of generated $\varepsilon$. 

```{r fig-nExp-yield, fig.width=7, fig.height=6, fig.cap = "Figure I.1: An example of experimental design of N and generated yield at the subplot resolution"}
field_m_error_low | field_m_error_high
```

\newpage

Using those yield datasets, we conducted one hundred simulations. The results of EONR estimation and yield prediction are shown in Table XX. (what about CNN reuslts??)


```{r report-table-optN-low, tab.cap = "Table I.1: Mean RMSE (kg/ha) estimation and profit-deficit ($/ha) by ML method and modeling scenario on low yield disturbance datasets"}
report_table_optN_low
```

```{r report-table-y-low, tab.cap = "Table I.2: Mean RMSE of yield prediction (kg/ha) by ML method and modeling scenario on training datasets on low yield disturbance datasets"}
report_table_y_low
```

\newpage

```{r report-table-optN-low, tab.cap = "Table I.3: Mean RMSE (kg/ha) estimation and profit-deficit ($/ha) by ML method and modeling scenario on low yield disturbance datasets"}
report_table_optN_high
```


```{r report-table-y-low, tab.cap = "Table I.4: Mean RMSE of yield prediction (kg/ha) by ML method and modeling scenario on training datasets on low yield disturbance datasets"}
report_table_y_high
```
