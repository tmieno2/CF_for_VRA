...
During the simulations, we evaluated the training accuracy using the same ML models we presented in the Result and Discussion section. 
Table D.1 shows the mean RMSE of the EONR estimation and $\hat{\pi}_{def}$ over the one thousand simulations using the training datasets as testing datasets. Overall, the performances of each ML method did not change between training and testing datasets. Regarding CNN, the identified site-specific yield response functions were perfectly linear as we confirmed in the test results. 
...
Table D.2 shows the mean RMSE of yield prediction over the one thousand simulations using the training datasets as testing datasets. Interestingly, the performances of the prediction-oriented ML methods in predicting yields were relatively improved compared with the test results. Despite the improvement of prediction-oriented ML methods in yield prediction accuracy, it did not result in the more accurate EONR estimates. This also supports our argument that the model’s yield prediction accuracy has little to do with its EONR prediction accuracy.
