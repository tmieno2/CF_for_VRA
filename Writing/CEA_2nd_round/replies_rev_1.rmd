...
We do see a redundancy in our explanation of how CF can be used to identify site-specific input recommendations. It was explained twice, once each in subsections 2.2 and 3.2.2. We have deletedthe explanation in 2.2 and also changed the heading in 2.1 to "Estimating EOIR and Treatment Effects."
...
First, in the new manuscript we consolidated the original manuscript's 3.1.2 and 3.1.3 into a single subsubsection called "Cell-specific Yield Functions and True Economically Optimal N Rates." In doing so, we moved to the supplementary appendix (Appendix B) the detailed explanation of the conceptual framework behind the yield response functions based on cell-level characteristics, $c_i$. In addition, we moved to Appendix C our detailed discussion of parameters and error term generation, which included a number of tables and figures. <span style = "color: blue;"> Shun put table and figure numbers here </span>.

Second, we consolidated subsection 3.1.4 and 3.1.5 of the original manuscript into a single subsubsection called "Trial Design and Yield Data Generation for Modeling." In doing so, we removed some text and combined the original manuscript's figures x and y in the single figure z in the current manuscript. 

Third, we moved to an appendix the previous manuscript’s detailed description of parameter splitting.

We believe that the detailed presentation of data generation is crucial in our reporting of MC procedures, as it would be in the report of any empirical study using real experiment data provides. In both cases, results are highly dependent procedures. We believe further slimming of the description would hinder anyone attempting to reproduce our results. That said, we have worked hard to remove redundancies in this section of the manuscript. 
</span>
...
<span style="color:blue">  
Shun is running simulations on training dataset and will report the results.
</span>
