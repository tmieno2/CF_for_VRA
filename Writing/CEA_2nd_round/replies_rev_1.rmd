...
It is degree. The unit is now provided in Appendix A.
...
Trainign dataset and test dataset are completely separate. Models are trained on the entirety of the train data for each simulation, and then the trained model is tested against a test dataset that are different in the underdyling field parameters $\alphe$, $\beta$, and $ymax$. The train dataset, however, was split into several parts when cross-validation was performed in determing the optimal hyperparameters at each simulation round.
