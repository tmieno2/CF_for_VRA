...
In essence, good performance in predicinting yield "levels" at the observed points does not mean good performanc ein predicting yield response function, which is about finding out what "would" happen to yield if rates other than the one actually applied were applied. This happens because you can predict yield leves for wrong reasons by giving wrong amount of credit to wrong drivers (variables) of yield levels. This is because prediction-oriented ML methods focus on predicting yield levels well at the "observed" points, but not the points that were not seen. This is by construction because they are trained to minimize the error in yield "level" prediction at the "observed" points. In contrast, CF focuses to recover the yield response functions well (or alternatively the causal impact of N on yield at different discrete rates of N). Our results how that this distinction between prediction-oriented ML methods and CF can result in economically meanigful difference, where CF outperforms.  
