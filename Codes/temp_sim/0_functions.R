...
sim_par <- function(i, reg_data, test_agg_data, N_levels) {
  ...
  # for example
  # x=1 
  # i=x ; reg_data = reg_data_all[sim==x&padding==1,]; test_agg_data = test_agg_data_all[sim==x&padding==1,]
  ...
  mutate(
      gam_formula = list(
          paste0(
           "opt_N_hat ~ ",
           # paste0("s(", var_ls, ", k = 5)", collapse = " + "),
           " s(X, k = 5) + s(Y, k = 5) + te(X, Y, k = c(5, 5))"
        ) %>% formula()
    )
  )%>%
  ungroup() %>%
  mutate(
      gam_res = mclapply(
        seq_len(nrow(.)),
        function(x) {
          gam(formula = .$gam_formula[[x]], data = .$opt_N_data[[x]])
        },
        mc.cores = detectCores() - 2
      )
    ) %>%
    ...
    mutate(
      opt_N_data = list(
        mutate(
          opt_N_data,
          opt_N_hat_g = predict(gam_res, newdata = opt_N_data)
        )
      )
    ) %>%
  select(-gam_formula, -gam_res) %>%
  unnest(., cols= "opt_N_data")%>%
  data.table()%>%
  setnames("unique_cell_id", "unique_subplot_id") %>%
  .[,.(sim, var_ls, model, unique_subplot_id, yield, opt_N, opt_N_hat, opt_N_hat_g, pred_yield)]
...
CF_analysis <- function(reg_data, var_ls, cl_var = NA) {
  ...
  # reg_data=reg_data; var_ls=c("X", "Y"); cl_var=NA
...
# BRF_analysis_temp <- BRF_analysis(test_agg_data=test_agg_data, brf_results=brf_temp, var_ls=var_ls, N_levels=N_levels)
