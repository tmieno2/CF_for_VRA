...
sim_par <- function(i, reg_data, test_agg_data, N_levels) {
  ...
  # for example
  # x=1 
  # i=x ; reg_data = reg_data_all[sim==x&padding==1,]; test_agg_data = test_data_all[sim==x&padding==1,]
...
get_opt_N <- function(reg_data, var_ls, cl_var, test_data_subplot, rates_ls, model) {
  ...
  if (model == "CF_stepwise") {
    ### === CF_stepwise ===###
    opt_N_data <- 
      CF_analysis(reg_data, var_ls, cl_var) %>%
      get_pi_dif(
    ...
    opt_N_data <- 
      CF_analysis_base(reg_data, var_ls, cl_var) %>%
      get_pi_dif_base(
        test_agg_data=test_data_subplot,
        cf_results=.,
        var_ls=var_ls,
    ...
    opt_N_data <- 
      MACF_run(reg_data, var_ls, cl_var) %>%
      MACF_analysis(
        test_data=test_data_subplot,
        macf_results=.,
        var_ls=var_ls,
        N_level=rates_ls
      ) %>%
  ...
  report_data <- opt_N_data[,.(unique_cell_id, opt_N_hat, pred_yield, sim, yield, opt_N)]
...
get_pi_dif <- function(test_agg_data, cf_results, var_ls, rates_ls) {
  # test_agg_data=test_agg_data; cf_results=cf_step
  ...
  pi_dif_data_agg <- lapply(
    ...
    .[test_agg_data, on = "unique_cell_id"]
...
get_pi_dif_base <- function(test_agg_data, cf_results, var_ls, rates_ls) {
  # test_agg_data=test_agg_data; cf_results=cf_base
  # var_ls=var_ls; rates_ls=N_levels
...
BRF_run <- function(reg_data, var_ls, cl_id = NA) {
  ...
  if (!is.na(cl_id)) {
    ...
    BRF_temp <- boosted_regression_forest(
      ...
      honesty = TRUE,
      clusters = cl,
      num.trees = 4000, # changed from 2000
      # min.node.size = 10,
      num.threads=1,
      tune.parameters = "all"
      # tune.parameters = c("sample.fraction", "mtry", "honesty.fraction", "honesty.prune.leaves", "alpha", "imbalance.penalty")
...
# test_agg_data = test_agg_data; brf_results = brf_temp; var_ls = var_ls; N_levels = N_levels 
...
# BRF_analysis_temp <- BRF_analysis(test_agg_data=test_data, brf_results=brf_temp, var_ls=var_ls, N_levels=N_levels)
...
RF_analysis <- function(test_agg_data, rf_results, var_ls, N_levels) {
  # test_agg_data = test_agg_data; rf_results = rf_temp; var_ls = var_ls; N_levels = N_levels 
  N_seq <- seq(min(N_levels), max(N_levels), by = 1)
...
# /*=================================================*/
#' # # Multi-arm causal forest (MACF) 
# /*=================================================*/
# x=1
# train <- reg_data_all[sim==x&padding==1,]
# test <- test_data_all[sim==x&padding==1,]
# var <- c("alpha", "beta", "ymax")
# N_levels <- reg_data_all[sim==x,]$rate%>%unique()%>%sort()

# MACF_temp <- MACF_run(reg_data=train, var_ls=var, cl_id = NA)
# MACF_res <- MACF_analysis(test_data=test, macf_results=MACF_temp, var_ls=var, N_level=N_levels)

MACF_run <- function(reg_data, var_ls, cl_id = NA){
  # reg_data = train; var_ls = var; cl_id = NA

  # === Preparation ===#
  X <- reg_data[, ..var_ls] # (5/25)changed from "rate" to "as_app_N"
  Y <- reg_data[, yield]
  W_f <- as.factor(reg_data[, aa_n])
  ...
  if (!is.na(cl_id)) {
    #--- if cl_id is present ---#
    MACF_temp <- multi_arm_causal_forest(
      X = X,
      Y = Y,
      honesty = TRUE,
      clusters = cl,
...
MACF_analysis <- function(test_data, macf_results, var_ls, N_levels) {
  # test_data = test; macf_results = MACF_temp
  # var_ls = var; N_levels = N_levels 
  
  X_test <- test_data[, ..var_ls]

  results <- 
    predict(macf_results, newdata = X_test)[[1]][, , 1] %>%
    data.table() %>%
    .[, unique_cell_id := test_data[, unique_cell_id]] %>%
    melt(id.var = "unique_cell_id") %>%
    .[, N := as.numeric(str_extract(variable, "^[^-]*"))] %>%
    .[, profit := pCorn * value - pN * N] %>%
    .[, .SD[which.max(profit), ], by = unique_cell_id] %>%
    .[profit < 0, N := min(N)] %>%
    .[, .(unique_cell_id, N)] %>%
    setnames("N", "opt_N_hat") %>%
    .[test_data, on = "unique_cell_id"]
...
####==== CF_stepwise ====####
# temp_CF_stepwise <- CF_analysis(reg_data=reg_data_sample, var_ls=c("alpha","beta","ymax"), cl_var = NA)


CF_stepwise_calculate_te <- function(test_data, cf_results, var_ls, rates_ls) {
  # test_data=test_data_sample; cf_results=temp_CF_stepwise
  # var_ls = c("alpha","beta","ymax"); rates_ls = N_levels

  cf_stepwise_te_data <- lapply(
    1:(length(cf_results) + 1),
    function(x) {
      get_changes_gradual(
        x,
  ...
  rbindlist() %>%
  setnames("N", "rate") %>%
  setnames("yield_dif", "tau_stepwise") %>%
  .[,.(unique_cell_id, rate, tau_stepwise)]

  return(cf_stepwise_te_data)
}



####==== CF_base ====####
# temp_CF_base <- CF_analysis_base(reg_data=reg_data_sample, var_ls=c("alpha","beta","ymax"), cl_var = NA)

CF_base_calculate_te <- function(test_data, cf_results, var_ls, rates_ls) {
  # test_data=test_data_sample; cf_results=temp_CF_base
  # var_ls = c("alpha","beta","ymax"); rates_ls = N_levels

  cf_base_te_data <- lapply(
    1:(length(cf_results) + 1),
    function(x) {
      get_changes_gradual_base(
        x,
  ...
  rbindlist() %>%
  setnames("N", "rate") %>%
  setnames("yield_dif", "tau_base") %>%
  .[,.(unique_cell_id, rate, tau_base)]

  return(cf_base_te_data)
}
...
BRF_calculate_te <- function(test_data, brf_results, var_ls, N_levels) {
  # test_data = test_data_sample; brf_results = temp_BRF
  # var_ls = c("alpha","beta","ymax"); N_levels = N_levels 
  
  brf_te_data <- test_data[, c("unique_cell_id", var_ls, "aa_n", "opt_N", "yield", "X", "Y"), with = FALSE] %>%
    .[rep(1:nrow(.), each = length(N_levels)), ] %>%
    .[, rate := rep(N_levels, nrow(.) / length(N_levels))] %>%
    .[, yield_hat := predict(brf_results, newdata = .[, c("rate", var_ls), with = FALSE])] %>%
    #--- Treatment effect calculation ---#
    .[, yield_base := .SD[1, yield_hat], by = .(unique_cell_id)] %>%
    .[, tau_base := yield_hat - yield_base] %>%
    .[, tau_stepwise := yield_hat - shift(yield_hat), by = unique_cell_id] %>%
    .[, .(unique_cell_id, rate, tau_base, tau_stepwise)]
...
####==== RF ====####
# temp_RF <- RF_run(reg_data=reg_data_sample, var_ls=c("alpha","beta","ymax"), cl_id = NA)


RF_calculate_te <- function(test_data, rf_results, var_ls, N_levels) {
  # test_data = test_data_sample; rf_results = temp_RF
  # var_ls = c("alpha","beta","ymax"); N_levels = N_levels 
  
  rf_te_data <- test_data[, c("unique_cell_id", var_ls, "aa_n", "opt_N", "yield", "X", "Y"), with = FALSE] %>%
    .[rep(1:nrow(.), each = length(N_levels)), ] %>%
    .[, rate := rep(N_levels, nrow(.) / length(N_levels))] %>%
    .[, yield_hat := predict(rf_results, newdata = .[, c("rate", var_ls), with = FALSE])] %>%
    #--- Treatment effect calculation ---#
