...
sim_par <- function(i, reg_data, test_agg_data) {
  print(paste0("working on ", i, " th iteration."))
...
get_opt_N <- function(reg_data, var_ls, cl_var, honesty, test_data_subplot, model) {
  ...
  # test_cell_agg <- rbind(test_data_subplot, test_data_cell)
  ...
  .[test_cell_agg, on = "unique_cell_id"]
    ...
    opt_N_data <- RF_run(
      ...
      RF_analysis(
        ...
        var_ls = var_ls)
    ### === Traditional ===###
  } else if (model == "Traditional") {
    opt_N_data <- Traditional_analysis(
      reg_data = reg_data,
      data_test = test_data,
      var_ls = var_ls,
      N_levels = rates_ls #changed from N_levels
    ) %>%
...
get_pi_dif <- function(test_agg_data, test_cell_data, cf_results, var_ls, rates_ls) {
  ...
  # /*-------------------------*/
  #' for cell testing data
  # /*-------------------------*/
  pi_dif_data_cell <- lapply(
    1:(length(cf_results) + 1),
    function(x) {
      get_changes_gradual(
        x,
        data_base = test_cell_data,
        var_ls = var_ls,
        rates_ls = rates_ls,
        cf_results = cf_results
      )
  ...
  pi_dif_data <- rbind(pi_dif_data_agg, pi_dif_data_cell)
...
BRF_run <- function(train_data, var_ls, honesty, cl_id = NA) {
  # train_data = reg_data[sim==1&padding==1,]
  # var_ls = c("alpha", "beta", "ymax")
  # honesty = TRUE
  # cl_id = NA

  # === Boosted random forest analysis ===#
  X <- train_data[, c("aa_n", var_ls), with = FALSE] 
  Y <- train_data[, yield]
...
BRF_analysis <- function(test_agg_data, brf_results, var_ls) {
  # test_agg_data = test_agg_data[sim==1&padding==1,]
  ...
  #' for cell testing data
  # /*-------------------------*/
  # eval_data_cell <- test_cell_data[, c("unique_cell_id", var_ls, "aa_n", "opt_N", "yield", "X", "Y"), with = FALSE] %>%
  #   ####==== for yield prediction ====####
  #   .[, pred_yield := predict(brf_results, newdata = .[, c("aa_n", var_ls), with = FALSE])]%>%
  #   ####==== for EONR estimation ====####
  #   .[rep(1:nrow(.), each = length(N_seq)), ] %>%
  ...
  #   .[,type:="cell"]
    ...
    # eval_data <- rbind(eval_data_agg, eval_data_cell)
...
#' # Run RF
# /*================================================================*/
RF_run <- function(train_data, honesty, var_ls, cl_id = NA) {

  # === Boosted random forest analysis ===#
  X <- train_data[, c("aa_n", var_ls), with = FALSE] # (5/25)changed from "rate" to "as_app_N"
  Y <- train_data[, yield]
...
RF_analysis <- function(test_agg_data, rf_results, var_ls) {
  # test_agg_data = test_agg_data[sim==1&padding==1,]
  # rf_results = rf_temp
  # var_ls = var_ls
